...
export default function AdminCourses() {
  ...
  const uploadThumbnail = async (file) => {
    ...
    try {
      ...
      const { data } = await axios.post('/admin/upload/thumbnail', formData, {
        headers: {
      ...
      setFormData(prev => ({ ...prev, thumbnail: `http://localhost:5000${data.url}` }))
    } catch (error) {
  ...
  const uploadMaterial = async (file) => {
    ...
    try {
      ...
      const { data } = await axios.post('/admin/upload/material', formData, {
        headers: {
          'Content-Type': 'multipart/form-data'
        }
      })
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Include materials in the course data
      const courseData = {
        ...formData,
        materials: materials
      }
      ...
      if (editingCourse) {
        ...
        const { data } = await axios.put(`/admin/courses/${editingCourse._id}`, courseData)
  ...
  return (
    <div className="min-vh-100" style={{ background: '#f8f9fa' }}>
            ...
            <div className="card border-0 shadow-sm">
              <div className="card-body p-4">
                ...
                {loading ? (
                  ...
                  <div className="text-center py-5">
                    <svg xmlns="http://www.w3.org/2000/svg" width="64" height="64" fill="currentColor" className="bi bi-book text-muted mb-3" viewBox="0 0 16 16">
                      <path d="M1 2.828c.885-.37 2.154-.769 3.388-.893 1.33-.134 2.458.063 3.112.752v9.746c-.935-.53-2.12-.603-3.213-.493-1.18.12-2.37.461-3.287.811V2.828zm7.5-.141c.654-.689 1.782-.886 3.112-.752 1.234.124 2.503.523 3.388.893v9.923c-.918-.35-2.107-.692-3.287-.81-1.094-.111-2.278-.039-3.213.492V2.687zM8 1.783C7.015.936 5.587.81 4.287.94c-1.514.153-3.042.672-3.994 1.105A.5.5 0 0 0 0 2.5v11a.5.5 0 0 0 .707.455c.882-.4 2.303-.881 3.68-1.02 1.409-.142 2.59.087 3.223.877a.5.5 0 0 0 .78 0c.633-.79 1.814-1.019 3.222-.877 1.378.139 2.8.62 3.681 1.02A.5.5 0 0 0 16 13.5v-11a.5.5 0 0 0-.293-.455c-.952-.433-2.48-.952-3.994-1.105C10.413.809 8.985.936 8 1.783"/>
                    </svg>
                    <p className="text-muted">
                      {searchTerm || categoryFilter ? 'No courses found matching your search.' : 'No courses available yet.'}
                    </p>
                  </div>
                ) : (
      ...
      {deleteCourse && (
        <div className="modal show d-block" style={{ background: 'rgba(0,0,0,0.5)' }} onClick={() => setDeleteCourse(null)}>
          <div className="modal-dialog modal-dialog-centered" onClick={(e) => e.stopPropagation()}>
            <div className="modal-content">
              <div className="modal-header bg-danger text-white">
                ...
                <button type="button" className="btn-close btn-close-white" onClick={() => setDeleteCourse(null)}></button>
              </div>
              <div className="modal-body">
                <p className="mb-0">
                  Are you sure you want to delete the course <strong>{deleteCourse.title}</strong>?
                </p>
                <p className="text-danger small mt-2 mb-0">
                  <strong>Warning:</strong> This action cannot be undone. All course data, including enrolled students and progress, will be permanently removed.
                </p>
              </div>
              <div className="modal-footer">
                <button type="button" className="btn btn-secondary" onClick={() => setDeleteCourse(null)}>
                  Cancel
                </button>
                <button
                  type="button"
                  className="btn btn-danger"
                  onClick={() => handleDeleteCourse(deleteCourse._id)}
                >
